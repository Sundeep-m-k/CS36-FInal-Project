...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False, label_encoder=None, point_encoder=None):
    ...
    def forward(self, samples: NestedTensor, points_supervision):
        ...
        query_embed = self.point_encoder(points_supervision, self.backbone.position_embedding, self.label_encoder)
        bs = len(query_embed)
    ...
    def _set_aux_loss(self, outputs_coord):
        ...
        return [{'pred_boxes': b}
                for b in outputs_coord[:-1]] #except last ..
...
def build(args):
    ...
    if args.dataset_file == 'cxr8': num_classes = 10 # 8+2
    ...
    criterion = PointCriterion(num_classes, weight_dict=weight_dict, eos_coef=args.eos_coef, losses=losses, args=args)
    criterion.to(device)
